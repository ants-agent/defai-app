...
import { AntsaiUser, PrismaUser, PrivyUser } from '@/types/db';

/**
 * Extended interface for AntsaiUser that includes Privy functionality
 * Omits 'user' and 'ready' from PrivyInterface to avoid conflicts
 */
type AntsaiUserInterface = Omit<PrivyInterface, 'user' | 'ready'> & {
  isLoading: boolean;
  user: AntsaiUser | null;
};

/**
 * Loads cached AntsaiUser data from localStorage
 * @returns {AntsaiUser | null} Cached user data or null if not found/invalid
 */
function loadFromCache(): AntsaiUser | null {
  try {
    const cached = localStorage.getItem('antsai-user-data');
    if (cached) {
...
/**
 * Saves AntsaiUser data to localStorage
 * @param {AntsaiUser | null} data User data to cache or null to clear cache
 */
function saveToCache(data: AntsaiUser | null) {
  try {
    if (data) {
      localStorage.setItem('antsai-user-data', JSON.stringify(data));
      debugLog('User data saved to cache', data, {
        module: 'useUser',
        level: 'info',
      });
    } else {
      localStorage.removeItem('antsai-user-data');
...
/**
 * Fetches AntsaiUser data from the server
 * @param {PrivyUser} privyUser The authenticated Privy user
 * @returns {Promise<AntsaiUser | null>} User data or null if fetch fails
 */
async function fetchAntsaiUserData(
  privyUser: PrivyUser,
): Promise<AntsaiUser | null> {
  try {
...
/**
 * Custom hook for managing AntsaiUser data fetching, caching, and synchronization
 * Combines Privy authentication with our user data management system
 * @returns {AntsaiUserInterface} Object containing user data, loading state, and Privy interface methods
 */
export function useUser(): AntsaiUserInterface {
  const { ready, user: privyUser, ...privyRest } = usePrivy();
  const [initialCachedUser, setInitialCachedUser] = useState<AntsaiUser | null>(
    null,
  );
   ...
   * @returns {Promise<AntsaiUser | null>} Combined user data or null
   */
  const fetcher = useCallback(async (): Promise<AntsaiUser | null> => {
    if (!ready || !privyUser) {
    ...
    if (privyUser) {
      debugLog('Fetching AntsaiUser data from server', null, {
        module: 'useUser',
        level: 'info',
      });
      const antsaiUser = await fetchAntsaiUserData(privyUser as PrivyUser);
      debugLog('Merged AntsaiUser data', antsaiUser, {
        module: 'useUser',
        level: 'info',
      });
      return antsaiUser;
    }
    debugLog('No valid AntsaiUser data retrieved', null, {
      module: 'useUser',
      level: 'warn',
    });
    return null;
  ...
  const { data: antsaiUser, isValidating: swrLoading } = useSWR<AntsaiUser | null>(
    swrKey,
    fetcher,
    {
  ...
  return {
    ...privyRest,
    isLoading: isLoading || antsaiUser == null,
    user: antsaiUser || null,
